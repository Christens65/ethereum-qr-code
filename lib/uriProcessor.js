...
var isValidEthAmount = function isValidEthAmount(amount) {
  var ethWeiAmount = new _bignumber2.default(amount);
...
var isAmountDefined = function isAmountDefined(val) {
...
var validateArgsDefaults = function validateArgsDefaults(argsDefaults, functionArgs) {
  ...
  if (argsDefaults.length === 0) {
    throw new Error('"argsDefaults" is an empty array. Do not use this prop if you do not have default amounts');
  }
  ...
  argsDefaults.forEach(function (argDefault) {
    ...
    if (matchedArg === null || typeof matchedArg === 'undefined') {
      throw new Error('"name" of the function argument with default amount does not fit function signature: ' + argDefault.name);
    }
  });
};
...
var validateUriSchemaFunction = function validateUriSchemaFunction(data) {
  validateAllowedProperties(data, ['to', 'from', 'amount', 'gas', 'mode', 'chainId', 'functionSignature', 'argsDefaults']);
...
var validateUriSchemaERC20Transfer = function validateUriSchemaERC20Transfer(data) {
  ...
  if (isAmountDefined(data.argsDefaults)) {
    validateArgsDefaults(data.argsDefaults, [{ name: 'to', type: 'address' }, { name: 'amount', type: 'uint' }]);
  }
};
...
var validateUriSchemaERC20Approve = function validateUriSchemaERC20Approve(data) {
  ...
  if (isAmountDefined(data.argsDefaults)) {
    validateArgsDefaults(data.argsDefaults, [{ name: 'spender', type: 'address' }, { name: 'amount', type: 'uint' }]);
  }
};
...
var validateUriSchemaERC20TransferFrom = function validateUriSchemaERC20TransferFrom(data) {
  ...
  if (isAmountDefined(data.argsDefaults)) {
    validateArgsDefaults(data.argsDefaults, [{ name: 'from', type: 'address' }, { name: 'to', type: 'address' }, { name: 'amount', type: 'uint' }]);
  }
};
...
var encodeEthSend = function encodeEthSend(data) {
  ...
  var params = {
    ...
    amount: isAmountDefined(amount) ? new _bignumber2.default(amount).toString(10) : null,
...
var decodeEthSend = function decodeEthSend(encodedStr) {
  var addressBlockParams = {
    gas: {
      convert: function convert(amount) {
        return parseInt(amount, 10);
      }
    },
    amount: {
      convert: function convert(amount) {
        return new _bignumber2.default(amount).toString(10);
      }
    },
    from: {
      convert: function convert(amount) {
